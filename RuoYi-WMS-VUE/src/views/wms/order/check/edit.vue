<template>
      ...
      <el-form>
        ...
        <el-form-item label="库区" prop="areaId">
          <el-select
            v-model="form.areaId"
            placeholder="请选择库区"
            :disabled="!form.warehouseId || checking"
            clearable
            filterable
          >
            <el-option
  ...
  <div v-else v-loading="loading">
          ...
          <el-table :data="form.details" border empty-text="暂无商品明细">
            <el-table-column label="商品信息" prop="itemSku.itemName">
              <template #default="scope">
                <div>
                  {{
                    scope.row.itemSku.item.itemName +
                ...
                <div v-if="scope.row.itemSku.item.itemBrand">
                  品牌：{{
                    useWmsStore().itemBrandMap.get(
                      scope.row.itemSku.item.itemBrand
                    ).brandName
                  }}
                </div>
              </template>
              ...
              <template #default="{ row }">
                ...
                <template v-else>
                  <div v-if="row.productionDate">
                    生产日期：{{ row.productionDate.substring(0, 10) }}
                  </div>
                  <div v-if="row.expirationDate">
                    过期日期：{{ row.expirationDate.substring(0, 10) }}
                  </div>
                </template>
              </template>
            </el-table-column>
            ...
            >
              <template #default="{ row }">
                <el-statistic
                  :value="Number(row.checkQuantity) - Number(row.quantity)"
                  :precision="0"
                />
              </template>
            </el-table-column>
            ...
            >
              <template #default="scope">
                <el-button
                  ...
                  plain
                  size="small"
                  v-if="scope.row.newInventoryDetail"
    ...
    <div class="footer-global" v-if="checking">
      <div class="btn-box">
        <div>
          <el-button @click="doCheck" type="primary" class="ml10"
            >完成盘库</el-button
          >
...
const close = () => {
  const obj = { path: "/inventory/checkOrder" };
...
const startCheck = () => {
  ...
  checking.value = true;
  loading.value = true;
  listInventoryDetailNoPage(query)
    .then((res) => {
      res.data.forEach((it) => {
        if (
          !form.value.details.find(
            (detail) => detail.inventoryDetailId === it.id
          )
        ) {
