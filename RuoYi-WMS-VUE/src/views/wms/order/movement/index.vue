    ...
    <el-card>
      ...
      >
        <el-form-item label="移库状态" prop="movementOrderStatus">
          <el-radio-group
            v-model="queryParams.movementOrderStatus"
            @change="handleQuery"
          >
        ...
        <el-form-item label="源仓库库区">
          <WarehouseCascader
            v-model:value="queryParams.sourcePlace"
        ...
        <el-form-item label="目标仓库库区">
          <WarehouseCascader
            v-model:value="queryParams.targetPlace"
            :show-all-levels="true"
            size="default"
            @keyup.enter="handleQuery"
          ></WarehouseCascader>
      ...
      >
          ...
          <template #default="props">
            <div style="padding: 0 50px 20px 50px">
              ...
              >
                ...
                <el-table-column
                  label="生产日期"
                  prop="inventoryDetail.productionDate"
                >
                  <template #default="{ row }">
                ...
                <el-table-column
                  label="过期日期"
                  prop="inventoryDetail.expirationDate"
                >
                  <template #default="{ row }">
        ...
        <el-table-column label="源仓库/源库区" align="left" width="260">
          <template #default="{ row }">
            <div>
              源仓库：{{
                useWmsStore().warehouseMap.get(row.sourceWarehouseId)
                  ?.warehouseName
              }}
            </div>
            <div v-if="row.sourceAreaId">
              源库区：{{
                useWmsStore().areaMap.get(row.sourceAreaId)?.areaName
              }}
            </div>
          </template>
        </el-table-column>
        <el-table-column label="目标仓库/目标库区" align="left" width="260">
          <template #default="{ row }">
            <div>
              目标仓库：{{
                useWmsStore().warehouseMap.get(row.targetWarehouseId)
                  ?.warehouseName
              }}
            </div>
            <div v-if="row.targetAreaId">
              目标库区：{{
                useWmsStore().areaMap.get(row.targetAreaId)?.areaName
              }}
            </div>
          </template>
        </el-table-column>
        <el-table-column
          label="移库状态"
          align="center"
          prop="movementOrderStatus"
          width="120"
        >
          <template #default="{ row }">
            <dict-tag
              :options="wms_movement_status"
              :value="row.movementOrderStatus"
            />
          </template>
        </el-table-column>
          ...
          <template #default="scope">
            <div>
              <el-popover
                ...
                :content="
                  '移库单【' +
                  scope.row.movementOrderNo +
                  '】已' +
                  (scope.row.movementOrderStatus === 1 ? '移库' : '作废') +
                  '，无法修改！'
                "
              >
                <template #reference>
                  <el-button
                    ...
                    v-hasPermi="['wms:movement:all']"
                    :disabled="[-1, 1].includes(scope.row.movementOrderStatus)"
                    >修改</el-button
                  >
                </template>
              </el-popover>
              ...
              >
                <template #reference>
                  <el-button
                    ...
                    v-hasPermi="['wms:movement:all']"
                    :disabled="scope.row.movementOrderStatus === 1"
                    >删除</el-button
                  >
                </template>
              </el-popover>
...
/** 新增按钮操作 */
function handleAdd() {
  proxy.$router.push({ path: "/inventory/movementOrderEdit" });
}
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm("确认删除移库单【" + row.movementOrderNo + "】吗？")
    .then(function () {
      loading.value = true;
...
function handleUpdate(row) {
  proxy.$router.push({
    path: "/inventory/movementOrderEdit",
    query: { id: row.id },
  });
}
...
async function handlePrint(row) {
  ...
  if (movementOrder.details?.length) {
    table = movementOrder.details.map((detail) => {
      return {
        ...
        sourceAreaName: useWmsStore().areaMap.get(detail.sourceAreaId)
          ?.areaName,
        targetAreaName: useWmsStore().areaMap.get(detail.targetAreaId)
          ?.areaName,
  ...
  const printData = {
    ...
    movementOrderStatus: proxy.selectDictLabel(
      wms_movement_status.value,
      movementOrder.movementOrderStatus
    ),
    sourceWarehouseName: useWmsStore().warehouseMap.get(
      movementOrder.sourceWarehouseId
    )?.warehouseName,
    sourceAreaName: useWmsStore().areaMap.get(movementOrder.sourceAreaId)
      ?.areaName,
    targetWarehouseName: useWmsStore().warehouseMap.get(
      movementOrder.targetWarehouseId
    )?.warehouseName,
    targetAreaName: useWmsStore().areaMap.get(movementOrder.targetAreaId)
      ?.areaName,
    totalQuantity: Number(movementOrder.totalQuantity).toFixed(0),
    createBy: movementOrder.createBy,
    createTime: proxy.parseTime(
      movementOrder.createTime,
      "{mm}-{dd} {hh}:{ii}"
    ),
    updateBy: movementOrder.updateBy,
    updateTime: proxy.parseTime(
      movementOrder.updateTime,
      "{mm}-{dd} {hh}:{ii}"
    ),
      ...
      if (res.data?.length) {
        const details = res.data.map((it) => {
          return {
            ...
            sourceAreaName: useWmsStore().areaMap.get(it.sourceAreaId)
              ?.areaName,
            targetWarehouseName: useWmsStore().warehouseMap.get(
              it.targetWarehouseId
            )?.warehouseName,
            targetAreaName: useWmsStore().areaMap.get(it.targetAreaId)
              ?.areaName,
          };
        });
